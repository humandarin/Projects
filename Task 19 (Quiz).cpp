...
/*
Сделано: викторина с вопросами в рандомном порядке, есть жизни и баллы и завершение с итогом, работающие настройки, кривые подсказки,
кривой вывод ответов с проверкой, кривые подсказки
Нужно сделать: ограничение по времени, нормальный рандом ответов
*/
...
int goQuiz() {
	...
	cout << "\x1b[90mВикторина началась!\x1b[0m\n";
	cout << "У вас \x1b[90m" << qtyLives << " \x1b[0mжизней.\n\n";

	for (int i = 0; i < qtyQuestions; i++) {
		cout << "\x1b[90m<" << i + 1 << ">\x1b[0m " << arrayQuestions[numQuestion[i]][0] << endl << endl;

		for (int j = 1; j < 5; j++) {
			cout << "\x1b[90m<" << numAnswer[j] << ">\x1b[0m " << arrayQuestions[numQuestion[i]][numAnswer[j]] << endl;
		}

		/*Если пользователь указывает ячейку, в которой содержится то же, что и в
		ячейке номер 5 - выводится Верно*/
		...
		if ((numAnswerUser == "0") && qtyLives >= 1) {
			cout << "Один из правильных вариантов ответа: " << rand() % 5 + 1 << " или " << arrayQuestions[numQuestion[i]][5];
			qtyHelps--;
			if (qtyHelps > 1) {
		...
		{
			if ((numAnswerUser == arrayQuestions[numQuestion[i]][5]) && (qtyLives >= 1) && (numAnswerUser != "0")) {
				qtyScores++;
				cout << "Верно!";
				//Проверка на баллы
				{
			...
			else if ((numAnswerUser != arrayQuestions[numQuestion[i]][5]) && (qtyLives >= 1) && (numAnswerUser != "0")) {
				qtyLives--;
				cout << "Неверно :с";
				//Проверка на жизни
				{
